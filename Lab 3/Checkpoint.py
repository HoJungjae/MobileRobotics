...
def handle_object_observed(evt, **kw):
    ...
    if isinstance(evt.obj, CustomObject):
        ...
        #print(f"{shared_data.marker_vect}")
...
async def run(robot: cozmo.robot.Robot):
    ...
    robot.set_head_angle(cozmo.robot.MIN_HEAD_ANGLE - cozmo.robot.MIN_HEAD_ANGLE).wait_for_completed()
    #Ignore cozmo logs
        ...
        while True:
            ...
            if event.image is not None:
                ...
                if shared_data.create_diamond is False:
                    shared_data.custom_cube = await robot.world.define_custom_cube(CustomObjectTypes.CustomType00, CustomObjectMarkers.Diamonds2, 50, 36, 36, True)

                if shared_data.cube2_start is True:
                    shared_data.custom_cube = await robot.world.define_custom_cube(CustomObjectTypes.CustomType00, CustomObjectMarkers.Triangles2, 50, 36, 36, True)
                ...
                # First cube moving
                if shared_data.marker_loc is not None and shared_data.task_complete is False:
                    angle = math.degrees(math.atan(shared_data.marker_loc[1] / shared_data.marker_loc[0]))
                    print(f"{shared_data.marker_loc[0]} {shared_data.marker_loc[1]}") # Debug code
                    if shared_data.marker_loc[0] > 115:
                        ...
                        shared_data.state_machine.send("cycle")
                        shared_data.face_display = "stop.png"
                        shared_data.task_complete = True
                else:
                    if shared_data.task_complete is False:
